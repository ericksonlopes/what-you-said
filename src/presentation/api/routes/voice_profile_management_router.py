...
async def register_new_voice_profile(
    request: VoiceProfileRegistrationRequest, db: Annotated[Session, Depends(get_db)]
):
    use_case = RegisterNewVoiceProfileUseCase(db)
    try:
...
async def train_voice_profile_from_existing_speaker_segment(
    ...
    use_case = TrainVoiceProfileFromSpeakerSegmentUseCase(db)
    try:
        voice_id = use_case.execute(
...
@router.get("")
async def list_all_registered_voice_profiles(db: Annotated[Session, Depends(get_db)]):
    use_case = ListRegisteredVoiceProfilesUseCase(db)
    return use_case.execute()


@router.delete("/{name}", responses={404: {"description": "Not Found"}})
async def delete_existing_voice_profile(
    name: str, db: Annotated[Session, Depends(get_db)]
):
    use_case = DeleteVoiceProfileUseCase(db)
    try:
