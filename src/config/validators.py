...
def docker_host_fallback(host: str, docker_names: set[str]) -> str:
    ...
    if host in docker_names and sys.platform == "win32" and not os.path.exists("/.dockerenv"):
        return "localhost"
    return host


def docker_host_fallback_optional(host: Optional[str], docker_names: set[str]) -> Optional[str]:
