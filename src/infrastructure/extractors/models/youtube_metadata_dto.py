...
class YoutubeMetadataDTO(BaseModel):
    ...
    original_url: Optional[str] = Field(
        default=None, description="URL original do vídeo"
    )
    title: Optional[str] = Field(default=None, description="Título do vídeo")
    full_title: Optional[str] = Field(
        default=None, description="Título completo do vídeo", alias="fulltitle"
    )
    ...
    duration_string: Optional[str] = Field(
        default=None, description="Duração formatada"
    )
    categories: Optional[List[str]] = Field(
        default=None, description="Categorias do vídeo"
    )
