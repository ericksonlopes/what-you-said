...
class TestIngestYoutubeUseCaseEdgeCases:
    ...
    def test_execute_playlist_no_url(self, use_case):
        ...
        with pytest.raises(ValueError, match="No video_url provided for playlist ingestion"):
            use_case.execute(cmd)
    ...
    def test_execute_any_failed_with_ingestion(self, use_case, mock_services):
        ...
        with patch.object(use_case, "_extract_video_id_from_url", return_value="12345678901"), \
    ...
    def test_process_single_video_job_reuse_not_found(self, use_case, mock_services):
        ...
        cmd = IngestYoutubeCommand(video_url="...", subject_id=str(uuid.uuid4()), ingestion_job_id=str(uuid.uuid4()))
        subject = MagicMock()
        
        with patch.object(use_case, "_create_ingestion_job") as mock_create:
            with patch("src.application.use_cases.ingest_youtube_use_case.YoutubeExtractor"):
