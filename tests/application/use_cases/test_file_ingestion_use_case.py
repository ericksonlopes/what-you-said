...
class TestFileIngestionUseCase:
    ...
    def test_execute_success(self, use_case_deps, mock_extractor):
        ...
        use_case_deps["ingestion_service"].create_job.return_value = MagicMock(id=job_id)
        ...
        mock_extractor.extract.return_value = [MagicMock(page_content="content", metadata={"source": "test.docx"})]
        use_case_deps["vector_service"].index_documents.return_value = ["vec1"]

        cmd = IngestFileCommand(file_path="/tmp/test.docx", file_name="test.docx", subject_id=subject_id)
    ...
    def test_execute_subject_not_found(self, use_case_deps):
        ...
        cmd = IngestFileCommand(file_path="/tmp/test.docx", file_name="test.docx", subject_id=uuid4())
    ...
    def test_determine_source_type_fallbacks(self, use_case_deps):
        ...
        def check(fname):
            return use_case._determine_source_type_refined(IngestFileCommand(file_name=fname))
    ...
    def test_execute_source_type_refinement(self, use_case_deps, mock_extractor, monkeypatch):
        use_case = FileIngestionUseCase(**use_case_deps)
        # Force it to be OTHER first so refinement triggers
        monkeypatch.setattr(use_case, "_determine_source_type_refined", lambda cmd: SourceType.OTHER)
        ...
        mock_extractor.extract.return_value = [MagicMock(page_content="content", metadata={"source_type": "pdf"})]
        use_case_deps["vector_service"].index_documents.return_value = ["vec1"]
    ...
    def test_execute_fallback_splitter(self, use_case_deps, mock_extractor, monkeypatch):
        ...
        mock_extractor.extract.return_value = [MagicMock(page_content="content line 1\ncontent line 2", metadata={})]
