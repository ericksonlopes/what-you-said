...
def test_more_sql_paths():
    ...
    cs_id = cs_repo.create(subject_id=ks_id2, source_type=SourceType.YOUTUBE.value, external_source="v_extra",
                           title="t", language="en")
    ...
    created = chunk_repo.create_chunks(
        [{"id": chunk_id, "content_source_id": cs_id, "job_id": job_id, "chunk_id": "filter-chunk", "chars": 1}])
...
def test_services_paths():
    ...
    cs = cs_service.create_source(subject_id=None, source_type=SourceType.YOUTUBE, external_source="svc-video",
    ...
    chunk_entity = ChunkEntity(job_id=job.id, content_source_id=cs.id, source_type=SourceType.YOUTUBE,
                               external_source="svc-video", subject_id=None, content=None)
