...
class TestAudioDiarizer:
    ...
    def mock_whisperx(self):
        with patch("src.infrastructure.services.whisperx_audio_diarizer.whisperx") as mock:
            self.whisperx = mock
            yield
    ...
    def test_run_diarization_pipeline(self, mock_torch, mock_exists, mock_load_audio):
        ...
        with patch.object(AudioDiarizer, "_transcribe") as mock_trans:
            mock_trans.return_value = ({"language": "en", "segments": []}, np.zeros(100))
            with patch.object(AudioDiarizer, "_align") as mock_align:
                ...
                with patch.object(AudioDiarizer, "_diarize") as mock_diarize:
                    ...
                    mock_diarize.return_value = ([Segment(start=0.0, end=1.0, text="test", speaker="S0")], "en")
    ...
    def test_transcribe_internal(self):
        ...
        with patch("src.infrastructure.services.whisperx_audio_diarizer.load_whisperx_audio", return_value=np.zeros(10)):
    ...
    def test_diarize_internal(self):
        with patch("src.infrastructure.services.whisperx_audio_diarizer.load_whisperx_audio", return_value=np.zeros(10)):
            ...
            with patch("whisperx.diarize.DiarizationPipeline", mock_pipeline_cls):
                self.whisperx.assign_word_speakers.return_value = {"segments": [{"start": 0, "end": 1, "text": "t", "speaker": "S1"}]}
                
                diarizer = AudioDiarizer(hf_token="fake")
                segments, _ = diarizer._diarize("f.wav", {"segments": []}, 1, None, None)
