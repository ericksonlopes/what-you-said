...
def mock_faiss(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules, "langchain_community.vectorstores.faiss", mock_lc_faiss
    )
    return FakeFAISS


def test_faiss_repository_create_and_retrieve(
    temp_index_path, embedding_service, mock_faiss
):
...
def test_faiss_repository_filtering(temp_index_path, embedding_service, mock_faiss):
    ...
    results = repo.retriever(
        query="document", top_kn=10, filters={"subject_id": str(subject_id)}
    )
...
def test_faiss_repository_delete_with_general_filter(
    temp_index_path, embedding_service, mock_faiss
):
    repo = ChunkFAISSRepository(
        embedding_service=embedding_service,
...
def test_faiss_repository_delete_no_filters_skips(
    temp_index_path, embedding_service, mock_faiss
):
    repo = ChunkFAISSRepository(
        embedding_service=embedding_service,
...
def test_faiss_repository_initialization_existing_index(
    temp_index_path, embedding_service, mock_faiss
):
...
def test_faiss_repository_error_handling(
    temp_index_path, embedding_service, mock_faiss
):
    repo = ChunkFAISSRepository(
        embedding_service=embedding_service,
    ...
    # Error in delete
    with patch.object(
        repo._vector_store, "delete", side_effect=Exception("Delete failed")
    ):
        with pytest.raises(Exception) as excinfo:
    ...
    with patch(
        "src.domain.mappers.chunk_mapper.ChunkMapper.document_to_model",
        side_effect=Exception("Map failed"),
    ):
        with pytest.raises(Exception) as excinfo:
